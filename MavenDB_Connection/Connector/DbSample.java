...
public class DbSample {
	...
	public void insert(int id,String name,String Phone_No,String age){
		try{
			PreparedStatement pd=con.prepareStatement("Insert into student(id,name,Phone_No,age)values(?,?,?,?)");
		pd.setInt(1, id);
		pd.setString(2, name);
	...
	public void select(String name,String age){
		try{
			PreparedStatement pd=con.prepareStatement("select * from student where name = '" + name + "' " + "and age = '" + age + "' ");
	...
	public void delete(int id){
		try{
			PreparedStatement pd=con.prepareStatement("delete from student where id='"+id+ "'");
			pd.executeUpdate();
		}catch(Exception e){
			System.out.println(e);
		}
	...
	public void update(int id,String Phone_No){
		try {
			PreparedStatement pd = con.prepareStatement("update student set Phone_No='"+Phone_No+"' "
															+"where id='"+id+"' ");
